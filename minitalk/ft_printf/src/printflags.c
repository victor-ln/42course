...
static int	zero_flag(t_params p, int len)
{
	...
	if (p.precision >= p.width && p.precision_c)
		return (printchar(p.precision, '0', len));
	if (p.precision < p.width && p.precision >= 0 && p.precision_c)
		return (printchar(p.width - p.precision, ' ', 0));
	if ((ft_strchr("xX", p.type) && p.hash) || (p.type == 'p'))
	{
		if (p.type == 'X')
			return (write(1, "0X", 2) + printchar(p.width, '0', len));
		return (write(1, "0x", 2) + printchar(p.width, '0', len));
	}
	return (printchar(p.width, '0', len));
}
...
int	print_precision(char *arg, t_params p)
{
	...
	if (p.precision <= len)
		return (0);
	if (minus)
		return (write(1, "-", 1) + printchar(p.precision - len, '0', 0));
	return (printchar(p.precision - len, '0', 0));
}
